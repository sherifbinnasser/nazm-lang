...
في لغة `نظم`، المتون هي السلاسل النصية التي تُكتب بين علامتي تنصيص مزدوجتين (`"`). المتون تُستخدم لتمثيل النصوص المكونة من سلسلة من الأحرف، سواء كانت أحرفًا أبجدية، أرقامًا، رموزًا، أو حتى مسافات وأحرف تحكم. تُمثَّل المتون كسلسلة من النوع [ط1](../tokens/numbers.md#الأعداد-الصحيحة).

```nazm
دالة البداية() {
    احجز كلام: [ط1] = "هذا متن"؛
}
```
